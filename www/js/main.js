...
(function(window, $, async) {
    ...
    var document = window.document, resources = window.resources = function() {
        var files = {};
    ...
    function formatTime(time) {
        return Math.floor(100 * time) / 100 + "s";
    }
    function createTableRow(resource) {
        return $(document.createElement("tr")).append(wrap(createPlayButton(resource), "td")).append(createTableCell(resource.name)).append(createTableCell(formatTime(resource.buffer.duration))).append(createTableCell(resource.buffer.sampleRate)).append(createTableCell(resource.buffer.numberOfChannels)).get(0);
    }
    function ResourcesController(el, context) {
        ...
        this.$el.on("change", '[type="file"]', this._triggerFileLoads.bind(this)).on("click", ".btn", this.playResource.bind(this));
        this._renderList();
    }
    ...
    ResourcesController.prototype.playResource = function(event) {
        var resource = resources.get(event.target.dataset.resourceKey), source = this.context.createBufferSource();
...
(function(window, $) {
    var document = window.document, MS_PER_S = 1e3, S_PER_MIN = 60, slice = Array.prototype.slice, STATES = {
        INACTIVE: 0,
        ACTIVE: 1
    };
    var Square = function() {
    ...
    function updateResourceSelectorOptions(resourceSelector) {
        var $el = $(resourceSelector), value = $el.val();
    ...
    function SequencerRow(width, context) {
        ...
        var resourceSelector = document.createElement("select"), squares = [], active, volumeControl = createVolumeInput(0, 90), source;
        updateResourceSelectorOptions(resourceSelector);
        this.output = context.createGainNode();
        this.output.gain.value = volumeControl.value / 100;
    ...
    window.controllers.Sequencer = function() {
        ...
        Sequencer.prototype._schedule = function() {
            var now = timestamp(), next = timePerBeat(this._tempo * 4);
            this._step();
...
(function(window, AudioContext, $, async, util) {
    var context = new AudioContext(), document = window.document;
    var LevelMeter = function() {
        ...
        LevelMeter.prototype.setLevel = function(level) {
            var db = util.toDB(level), gradient = this._createGradient();
            this.fill("#555");
    ...
    $(function() {
        var sequencer = new window.controllers.Sequencer(3, context), meter = window.meter = new LevelMeter(context, {
            width: 800,
            height: 25,
            orientation: LevelMeter.HORIZONTAL
        }), controls = new window.controllers.SequencerControls(document.getElementById("controls"), sequencer);
